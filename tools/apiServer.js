...
const router = jsonServer.router(path.join(__dirname, "db.txt"));
...
server.post("/contacts/", function(req, res, next) {
  ...
  if (error) {
    ...
    req.body.slug = createSlug(req.body.fName+req.body.lName); // Generate a slug for new contacts.
    next();
  }
});
...
function validateContact(contact) {
  if (!contact.fName) return "First name is required.";
  if (!contact.lName) return "Last name is required.";
